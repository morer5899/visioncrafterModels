...
# Initialize Gradio client for background removal with error handling
try:
    bg_removal_client = Client("not-lain/background-removal")
    logger.info("Background removal client initialized successfully")
except Exception as e:
    logger.error(f"Failed to initialize background removal client: {str(e)}")
    bg_removal_client = None

# Initialize rembg if available
try:
...
def remove_bg():
    ...
    # If rembg not available or failed, try Gradio client
    if bg_removal_client is not None:
        try:
            image_file = request.files['image']
    ...
    return jsonify({
        "error": "Background removal service is currently unavailable",
